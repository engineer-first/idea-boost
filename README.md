...
Node.js の準備と `npm ci` の後、通常の開発サーバーを止めて実行します。
デモには `.env.local` / `workers/.dev.vars` の作成や Google ログインは不要です。

```bash
npm run demo
```

表示された <http://localhost:3000/demo> を開き、見せ場を選んで「デモを開始」を押します。
あなたがホスト、残り4人がデモ参加者として準備されたルームに入ります。
通常の `npm run dev` ではデモは無効なため、`/demo` は404になります。
題材は「学生生活の困りごとを解決するアプリをハッカソンで考える」です。

- **見せ場**: 1-1から3-5まで全14ステップから開始できます。1-3は名前付きの2グループ、HMW・アイデアも具体例を準備済みです。
- **付箋**: 各フェーズに8枚。5人全員が1〜2枚を持ち、ホストも各フェーズの開始時に自分の下書きが届きます。
- **デモ操作パネル**: 目的・自動準備・手動操作・入力例・話す内容をステップ別に表示します。他4人の共有・投票、1-3のグループ例配置を合図で実行できます。自分の共有・投票・採用は手動で行います。
- **自由操作**: 自分で付箋の追加・共有・投票・採用案の選択ができます。4人の発言は固定なので、台本と違う課題を選ぶと後続の内容とはつながらない場合があります。
- **やり直し**: 見せ場の初期状態を持つ新しいデモルームに移ります。自由操作した内容は引き継ぎません。
- **終了**: ターミナルで `Ctrl+C`。APIとNextをまとめて停止します。

デモのDBは `.wrangler/demo/state` に保存し、通常開発のDBとは分離します。
セッション鍵と操作用の鍵は起動ごとに生成し、終了時に一時設定を削除します。
通常の開発に戻った際はログインし直してください。
3000 / 8787 番ポートを使うため、通常開発との同時起動は行いません。
通常の起動・本番環境ではデモ機能を利用できません。

全14ステップの台本、グループ名の入力例、全24枚のサンプル一覧は
