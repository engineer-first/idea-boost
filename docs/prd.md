...
- **画面下から1件を最終決定する**：Step 1-5・2-4・3-5の採用前は、ホストに「もう一度投票する」「採用する付箋を選ぶ」を下中央に示す。選択開始後にボードの候補をクリックし、本文・作者と、決定後は再投票・候補整理・移動・取消ができないことを確認して確定する。空白クリックやドラッグ終了は選択に扱わず、キャンセル・Escape・ステップ変更・切断で選択を解除する。個人・候補外・別フェーズの付箋は対象外とする。RoomDOがホスト・現在フェーズ・共有候補・未確定を再検証し、全員へ同期する。最多票の自動採用は行わず、1位以外や候補1件も直接決定できる。確定後は下中央の2操作を消し、フェーズ1・2は右上から次フェーズへ、フェーズ3は全員にスプリント完了を示す。
