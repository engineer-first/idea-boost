...
- **決定時に候補を一時的に外せる**：Step 1-5・2-4・3-5ではホストだけが確認なしで付箋を候補外にでき、直後の Undo、右クリック、ホバー・フォーカス・タップから同じ位置へ戻せる。候補外でも本文・票・座標・大きさ・グループ・マップ位置を保持し、全員に同じ場所のゴーストとして表示する。候補数・集計・決定対象からは除き、候補が0件なら決定・次フェーズ進行を止める。別一覧、一括除外、完全削除、複数選択、自由移動は設けない。
