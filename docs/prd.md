...
- **1280×720で主要操作と補助パネルを重ねない**：上部左側の現在地は最大360pxとし、フェーズ名・正式なステップ名・進捗を表示する。アプリ名やフェーズ番号などの重複する見出しは置かない。右上は参加者・タイマー・ホストの招待ボタン・次ステップへの操作・ルームメニューを一つの不透明な操作バーにまとめる。招待URLとコードは「招待」から開き、ルームメニューには退出・解散などの操作を置く。現在地に開閉できる詳細領域をつなぎ、「進め方」と「決定事項」を切り替える。決定した課題・HMWはこの領域で全文を読み、長文だけを内部スクロールする。横幅いっぱいの決定内容バーは常設しない。詳細と考えるヒントは左側で縦に並び、詳細を閉じるとヒントの表示領域が広がる。マイ付箋は右に置き、必要なパネルだけを内部スクロールする。カーソル表示の切替は左下の操作可否の近くに置く。マイ付箋のパネルは幅240pxとし、幅192pxの付箋の左右に操作に必要な余白を残す。狭い画面では一部の参加者アイコンを省略し、進捗バーは短くして表示を続ける。参加者を省略するときはアバターと同じ大きさの丸に省略人数（例：`+9`）を表示し、クリックで全員の一覧を開けるようにする。768px以上では現在地と操作群を1段に収める。タイマーの寸法と次ステップの操作を維持し、現在地の文字を押し潰さない。
