...
- **大枠フェーズは3つに整理する**：デザインスプリントの大枠フェーズは「課題整理」「問いの作成」「アイデア出しの評価決定」の3つとする。
...
- **1280×720で主要操作と補助パネルを重ねない**：上部左側の現在地は最大360pxとし、フェーズ名・正式なステップ名・進捗を表示する。アプリ名やフェーズ番号などの重複する見出しは置かない。右上は参加者・タイマー・ホストの招待ボタン・次ステップへの操作・ルームメニューを一つの不透明な操作バーにまとめる。招待URLとコードは「招待」から開き、ルームメニューには退出・解散などの操作を置く。現在地の下に「進め方」「決定したHMW」「決定した課題」を縦に並べ、それぞれ独立して開閉する。進め方と決定事項は同時に参照でき、進め方だけを閉じても採用した内容は残る。HMW作成（Step 2-1）では決定した課題、アイデア作成（Step 3-1）では決定したHMWを最初から開き、フェーズ3の元の課題は必要に応じて開けるようにする。それ以外のステップでは参照欄を閉じて始める。各欄で全文を読め、長文はそれぞれ内部スクロールする。横幅いっぱいの決定内容バーは常設しない。参照欄と考えるヒントは左側で縦に並び、不要な欄を閉じるとヒントの表示領域が広がる。マイ付箋は右に置き、必要なパネルだけを内部スクロールする。カーソル表示の切替は左下の操作可否の近くに置く。マイ付箋のパネルは幅240pxとし、幅192pxの付箋の左右に操作に必要な余白を残す。狭い画面では一部の参加者アイコンを省略し、進捗バーは短くして表示を続ける。参加者を省略するときはアバターと同じ大きさの丸に省略人数（例：`+9`）を表示し、クリックで全員の一覧を開けるようにする。768px以上では現在地と操作群を1段に収める。タイマーの寸法と次ステップの操作を維持し、現在地の文字を押し潰さない。
- **補助情報は必要なステップで開く**：考えるヒントは Step 2-1と3-1で展開して始める。Step 3-1は「書き出し」「発想を広げる」を同じパネルのタブで切り替える。Step 3-2では参考資料として折り畳み、それ以外では表示しない。マイ付箋は折り畳んで始め、追加時に開く。左右の開閉は独立し、閉じても入力内容やキャンバスの表示位置を変えない。
