...
- **1280×720で主要操作と補助パネルを重ねない**：上部に現在地と操作群、その下に決定した課題・HMWを各1行で表示し、長文は全文表示で読める。ガイドと考えるヒントは左、マイ付箋は右に置き、必要なパネルだけを内部スクロールする。カーソル表示の切替は左下の操作可否の近くに置く。マイ付箋のパネルは幅240pxとし、幅192pxの付箋の左右に操作に必要な余白を残す。狭い画面ではロゴと一部の参加者アイコンを省略し、進捗バーは短くして表示を続ける。参加者を省略するときはアバターと同じ大きさの丸に省略人数（例：`+9`）を表示し、クリックで全員の一覧を開けるようにする。1024px未満では上部を2段にする。タイマーの寸法と次ステップの操作を維持し、現在地の文字を押し潰さない。
