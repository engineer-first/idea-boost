...
- **ファシリテーションガイドが各ステップを案内**：ステップに入ると画面中央の説明モーダルを表示し、目的・手順・具体例・完了の目安を示す。ホストにだけ進行指示を追加表示する。モーダルの主ボタンは、そのステップの最初の安全な操作へつなげてから閉じる。閉じた後は現在地HUDの「進め方」から再表示できる
...
- **画面下から1件を採用する**：Step 1-5・2-4・3-5ではホストに「採用する付箋／問い／アイデアを選ぶ」入口を画面下に示す。選択中は候補だけを明示し、対象を直接クリックして1件を確定する。空白クリックやドラッグ終了は確定に扱わず、キャンセル・Escape・ステップ変更・切断で選択モードを解除する。個人・候補外・別フェーズの付箋は対象外とする。正式決定と解除は RoomDO を唯一の真実とし、ホスト・現在フェーズ・共有候補をサーバーで再検証し、全員へ同期する。決定内容は全員が確認でき、ホストは次ステップへ進む前に解除できる。
- **決定時に候補を一時的に外せる**：Step 1-5・2-4・3-5ではホストだけが、個別の付箋を確認なしで候補外にできる。直後の Undo、右クリック、ホバー・フォーカス・タップから同じ位置へ戻せる。さらに、主観票・客観票ともに投票のない共有済み候補は、対象件数を示した確認を経て一括で候補外にできる。対象は実行時にサーバーで再判定し、一括操作で実際に外した候補のうち現在も同じ操作由来で候補外のものだけをまとめて元へ戻せる。候補外でも本文・票・座標・大きさ・グループ・マップ位置を保持し、全員に同じ場所のゴーストとして表示する。候補数・集計・決定対象からは除き、候補が0件なら決定・次フェーズ進行を止める。別一覧、任意の複数選択、完全削除、自由移動は設けない。
