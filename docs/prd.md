...
- **タイマーは推奨時間から手動で開始する**：各ステップの推奨時間をタイマーの初期値にするが、自動開始・自動進行は行わない。実行中は「＋1分」「一時停止」、一時停止中は「再開」「終了」の2操作だけを表示し、手動終了・時間切れは共有された終了状態として00:00と「時間になりました。」を示す。「もう一度設定」から再設定できるが自動再開はしない。成功したステップ移行ではタイマーを停止し、次ステップの推奨時間へ戻す。
