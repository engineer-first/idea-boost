...
>
> - 1画面 = 1フェーズ
> - 現在地・ゴールは常時表示し、全手順は「全手順を見る／閉じる」で開閉する。基本ルートに沿いながら、追加の個人作業と再投票を繰り返せる（[ステップ別機能対応表](./ds-feature-matrix.md#同じフェーズ内での繰り返し)）
> - 各フェーズの最後に「成果物を決定」する
...
初めて訪れたステップは画面上中央の短い案内を表示し、約5秒後に「進め方」へ畳む。詳細な手順・具体例・進行役案内は手動で開ける。追加の個人作業や共有への再訪では説明を強制表示せず、同じ作業位置から続けられる。詳しい表示規則は[PRD](./prd.md#62-設計コンセプト)に従う。
...
> 候補を動かして話し合い、必要ならもう一度投票してください。採用する付箋を選び、本文・作者と決定後の制限を確認して最終決定してから、次フェーズへ進んでください。
...
全参加者が候補を移動できる。投票中は位置を固定し、Step5では最終決定までマップ内で位置を見直せる。
...
> 必要なら候補を動かして話し合い、もう一度投票してください。採用する付箋の本文・作者と決定後の制限を確認して最終決定すると、全員にスプリント完了を表示します。
