...
フェーズ1 Step 1へ入ると、説明モーダルを中央に表示する。内容は中央揃えとし、上から「デザインスプリントを始めよう！」、大見出し「最近あった困ったことを付箋に書き出そう。」、課題の例を2件、ホストのみの進行役案内、青いCTA「付箋に課題を書く」を表示する。CTAを押すとモーダルを閉じ、個人付箋を作成して入力欄へフォーカスする。中央モーダルを閉じた後、「進め方」を押すと同じ説明を左上のパネルに左揃えで表示するが、パネルにはCTAを表示しない。
...
ステップに入ると、中央モーダルに「作成した付箋をメンバーに共有しよう！」と表示する。その下に「やること」として、共有する順番を決めること、最初の人が付箋を一つずつ説明しながら共有すること、決めた順番で次の人が発表することを表示する。ホストには進行役向け案内を表示する。
...
ステップに入った時点で、説明を画面中央のモーダルに表示する。モーダルには目的、手順、具体例、完了の目安をまとめ、主ボタン「最初の問いを書く」から個人付箋を1枚作成して入力を開始できるようにする。モーダルを閉じた後も、現在地HUDの「進め方」から再表示できる。
...
- 縦軸：価値（上に行くほど高い。選定した課題やHMWに対して効果的なアイデアかを判断）
- 横軸：実現可能性（右に行くほど高い）
