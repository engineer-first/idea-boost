...
フェーズ1 Step 1へ入ると、説明モーダルを中央に表示する。内容は中央揃えとし、上から「最初の一歩」、大見出し「課題を1つの問いに言い換えてみよう」、課題の例を2件、ホストのみの進行役案内、青いCTA「付箋に課題を書く」を表示する。CTAを押すとモーダルを閉じ、個人付箋を作成して入力欄へフォーカスする。
