...
export function VoteTotalingPanel({
  ...
  return (
    <section
      className="mx-auto w-full max-w-3xl bg-transparent p-5 text-foreground sm:p-8"
      aria-label="投票結果"
    ...
    >
      ...
      <aside
        aria-label="配点の説明"
        className="mt-5 rounded-lg bg-muted/60 p-3 text-left text-sm text-muted-foreground"
        data-testid="vote-totaling-scoring-guide"
      >
        <p className="font-semibold text-foreground">配点</p>
      ...
      {result.rows.length === 0 ? (
        <p
          className="mt-6 rounded-lg bg-muted/40 px-4 py-8 text-center text-sm text-muted-foreground"
