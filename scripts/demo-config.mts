...
export async function prepareDemoRuntime(
  ...
  const demoDir = join(projectDir, ".wrangler/demo");
  await mkdir(demoDir, { recursive: true, mode: 0o700 });
  const directory = await mkdtemp(join(demoDir, "runtime-"));
  const configPath = join(directory, "wrangler.json");
  const persistPath = join(demoDir, "state");
  const secret = randomBytes(32).toString("hex");
  const token = randomBytes(32).toString("hex");
  const config = {
    name: "idea-flow-demo",
    main: join(projectDir, "workers/demo-worker.ts"),
    compatibility_date: "2026-06-01",
    compatibility_flags: ["nodejs_compat"],
    dev: { ip: "127.0.0.1", port: 8787 },
    durable_objects: {
      bindings: [{ name: "ROOM_DO", class_name: "DemoRoomDO" }],
    },
    migrations: [{ tag: "v1", new_sqlite_classes: ["DemoRoomDO"] }],
    d1_databases: [
      {
        binding: "DB",
        database_name: "idea-flow-demo",
